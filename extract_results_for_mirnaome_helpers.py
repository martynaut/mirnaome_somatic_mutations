...
def retract_counts(normal, tumor, data_format, ref, alt):
    ...
    if 'BCOUNT' in data_format.values[0]:
        ...
        norm_bcount = normal.values[0].split(':')[4].split(',')
        tumor_bcount = tumor.values[0].split(':')[4].split(',')
    ...
    # varscan2
        ...
        norm_rd = normal.values[0].split(':')[3]
        norm_ad = normal.values[0].split(':')[4]
        norm_ref = int(norm_rd)
        norm_alt = int(norm_ad)
        tumor_rd = tumor.values[0].split(':')[3]
        tumor_ad = tumor.values[0].split(':')[4]
        tumor_ref = int(tumor_rd)
        tumor_alt = int(tumor_ad)

    # muse & mutec2
    elif 'AD' in data_format.values[0]:
        norm_ad = normal.values[0].split(':')[1].split(',')
        tumor_ad = tumor.values[0].split(':')[1].split(',')
    ...
    if 'SSC' in data_format.values[0]:
        ssc = float(tumor.values[0].split(':')[-1])

    if 'BQ' in data_format.values[0]:
        bq_index = data_format.values[0].split(':').index('BQ')
        if bq_index == 2:
            bq_ref_tum, bq_alt_tum = tumor.values[0].split(':')[bq_index].split(',')[:2]
            bq_ref_norm, bq_alt_norm = normal.values[0].split(':')[bq_index].split(',')[:2]
    ...
    if 'QSS' in data_format.values[0]:
        qss_ref_tum, qss_alt_tum = tumor.values[0].split(':')[-3].split(',')[:2]
        qss_ref_norm, qss_alt_not = normal.values[0].split(':')[-3].split(',')[:2]
